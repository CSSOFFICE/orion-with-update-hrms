$(document).ready(function () {
	...
	jQuery("#invoice_template").submit(function (e) {
		/*Form Submit*/
		...
		fd.append("is_ajax", 7);
		fd.append("data", "invoice_template");
		fd.append("type", "invoice_template");
	...
	jQuery("#quote_settings").submit(function (e) {
		/*Form Submit*/
		var fd = new FormData(this);
		var obj = $(this),
			action = obj.attr("name");
		fd.append("form", action);
		fd.append("is_ajax", 7);
		fd.append("data", "quote_settings");
		fd.append("type", "quote_settings");
		...
		jQuery.ajax({
			...
			success: function (JSON) {
				if (JSON.error != "") {
					...
					toastr.success(JSON.result);
					$('input[name="csrf_hrsale"]').val(JSON.csrf_hash);
					$('#logo1').attr("src", JSON.logo1);
					$('#logo2').attr("src", JSON.logo2);
					$('#logo3').attr("src", JSON.logo3);
					$('#logo4').attr("src", JSON.logo4);
					$('#logo5').attr("src", JSON.logo5);
					$('#logo6').attr("src", JSON.logo6);
					$('#logo7').attr("src", JSON.logo7);
					$('#quote_settings')[0].reset();
					// $(".icon-spinner3").hide();
					// jQuery(".save").prop("disabled", false);
				}
			},
