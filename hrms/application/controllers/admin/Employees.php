	...
	public function add_employee()
	{
		...
		if ($this->input->post('add_type') == 'employee') {
			...
			if ($pm_result) {
				...
				//Upload Function for Signature
				if (is_uploaded_file($_FILES['sig']['tmp_name'])) {
					//checking image type
					...
					if (in_array($ext, $allowed)) {
						$tmp_name = $_FILES['sig']["tmp_name"];
						$profile = "uploads/document/signature/";
						$set_img = base_url() . "uploads/document/signature/";
						// basename() may prevent filesystem traversal attacks;
						// further validation/sanitation of the filename may be appropriate
						$name = basename($_FILES['sig']["name"]);
						$newfilename = 'signature_file_' . round(microtime(true)) . '.' . $ext;
					...
					$this->db->update('xin_employees', $s_data, ['user_id' => $pm_result]);
				}
