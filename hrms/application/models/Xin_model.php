...
class Xin_model extends CI_Model
{
	...
	public function select_module_class($mClass, $mMethod)
	{
		...
		if ($mClass == 'dashboard') {
			...
			$arr['stff_open'] = 'active';
			return $arr;
		}else if ($mClass == 'employeebenefits') {
			$arr['emp_benefits_active'] = 'active';
			$arr['stff_open'] = 'active';
			return $arr;
...
ORDER BY `expiry_date`");
	...
	{
		$query = $this->db->where('country_id', $id)->get("xin_countries");
		return $query->result();
	}
	public function get_gst()
	{
		return $this->db->get('xin_gst');
	}
	public function get_payment_term()
	{
		return $this->db->get('xin_payment_term');
	}
	public function get_unit()
	{
		return $this->db->get('xin_unit');
	}
	public function add_term_condition($data)
	{
		$this->db->insert('xin_term_condition', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		} else {
			return false;
	...
	{
		return $this->db->get('xin_term_condition');
	}
	public function read_term_condition($id)
	{
		$sql = 'SELECT * FROM xin_term_condition where term_id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
	...
	public function update_term_condition($data, $id)
	{
		...
		if ($this->db->update('xin_term_condition', $data)) {
			return true;
		} else {
			return false;
		}
	...
	{
		$this->db->where('term_id', $id);
		$this->db->delete('xin_term_condition');
	}
	public function get_quotation_term($id)
	{
	...
	public function get_task_list(){
		return $this->db->get('xin_cons_tasks');
	}
	public function add_task($data){
		$this->db->insert('xin_cons_tasks', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		} else {
			return false;
	...
	public function update_task_record($data,$id){
		...
		if ($this->db->update('xin_cons_tasks', $data)) {
			return true;
		} else {
			return false;
		}
	...
	public function delete_task_record($id){
		$this->db->where('task_id', $id);
		$this->db->delete('xin_cons_tasks');		
	}
	public function get_overtime_type_list(){
		return $this->db->get('xin_overtime_type');
	}
	public function add_overtime_type($data){
		$this->db->insert('xin_overtime_type', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		} else {
			return false;
	...
	public function read_overtime_task($id){
		$sql = 'SELECT * FROM xin_overtime_type WHERE overtime_type_id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
	...
	public function update_overtime_type($data,$id){
		$this->db->where('overtime_type_id', $id);
		if ($this->db->update('xin_overtime_type', $data)) {
			return true;
		} else {
			return false;
		}
	}
	public function delete_overtime_type_record($id){
		$this->db->where('overtime_type_id', $id);
		$this->db->delete('xin_overtime_type');
	}

	public function get_packing_type()
	{
		return  $query = $this->db->query("SELECT * from xin_packing_type");
	}

	public function payment_methods()
	{
		return  $query = $this->db->query("SELECT * from xin_payment_method")->result();
	}
