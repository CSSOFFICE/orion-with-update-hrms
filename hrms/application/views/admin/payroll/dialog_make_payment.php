...
if (isset($_GET['jd']) && isset($_GET['employee_id']) && $_GET['data'] == 'payment' && $_GET['type'] == 'monthly_payment') { ?>
	...
	// echo $database_cpf_employee;
	$database_cpf_employee = $final_calculation['cpf_employee'] - $database_cpf_employee;


	// logic for hourly payment
	//overtime request
	$overtime_count = $this->Overtime_request_model->get_overtime_request_count($user_id, $pay_date);
	$re_hrs_old_int1 = 0;
	...
	if ($check && $check[0]->is_advance == 1) {
		$total_net_salary = $total_net_salary - $final_calculation['contribution'];
		$total_contribution = $final_calculation['contribution'];
	} else if ($check && $check[0]->is_advance != 1) {
		$total_contribution = $final_calculation['contribution'];
	} else {
					...
					<div class="tab-content active" id="tab1">
						<div class="row" style="margin: 5px;">
							...
							<?php if ($wages_type == 2) { ?>
								<div class="col-md-6">
									<div class="form-group">
										<label for="leave_deductions"> Working Hour </label>
										<input type="text" name="total_working_hour" id="total_working_hour" class="form-control ontim_data_change" value="<?php echo $pcount; ?>">
									</div>
								</div>
							<?php } ?>
							<div class="col-md-<?php echo $wages_type == 2 ? 12 : 6 ?>">
								<div class="form-group">
						...
						<div class="row" style="margin: 5px;">
							...
							<div class="col-md-6">
								<div class="form-group">
									...
									<input type="text" name="total_fund_contribution" id="total_fund_contribution" class="form-control" value="<?php echo $total_contribution; ?>">
								</div>
							</div>
						</div>
						<div class="row" style="margin: 5px;">
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$('.bc_checked_class').click(function() {
				...
				$.ajax({
					url: "<?php echo base_url('admin/payroll/check_payment') ?>",
					type: 'get',
					dataType: 'json',
					data: {
						check: check_value,
					...
					success: function(data) {
						...
						$('#total_statutory_deductions').val(data.statutory_deductions);
					},
