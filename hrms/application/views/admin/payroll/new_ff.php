		...
		<tbody>
			...
			<tr class="va-top">
				...
				<td>
					...
					<p class="blue-text" style="padding-left: 3%;">
						<?php
						foreach ($payslip_detail as $p) { ?>
							<?= date('d-m-Y', strtotime($p->year_to_date)) . " / " ?>
						<?php } ?>
					</p>
				</td>
			</tr>
			...
			<tr class="va-top">
				<td>
					<p class="td-head">
						Name of Employee</p>
					<p class="blue-text" style="padding-left: 3%; font-size: 28px;"><?= $name ?></p>
				</td>
				<td></td>
				<td>
					...
					<table class="bordered-table-d" style="font-size: 25px;">
						<tbody>
							...
							<tr>
								...
								<td class="blue-text"><?= $this->Xin_model->currency_sign($basic_pay, $user_id) ?></td>
							...
							<tr>
								...
								if ($allowances):
									foreach ($allowances as $al):
										$allowance_total += $al->allowance_amount;
									endforeach;
								endif;
								?>
							...
							<tr>
								...
								<td class="blue-text"><?= $this->Xin_model->currency_sign($basic_pay + $allowance_total, $user_id) ?></td>
							...
							<tr>
								...
								if ($loans):
									foreach ($loans as $loan):
										$total_deduction += $loan->loan_amount;
									endforeach;
								endif;
								if ($statutory_deductions):
									foreach ($statutory_deductions as $statutory_deduction):
										$total_deduction += $statutory_deduction->deduction_amount;
									endforeach;
								endif;
								if ($leave_deduction):
									$total_deduction += $leave_deduction;
								endif;
								?>
								<td class="bg-second">Total Deductions<br>
									<span style="font-size: 20px;font-style: italic;">(Breakdown shown below)</span>
								</td>
								<td class="blue-text"><?= $this->Xin_model->currency_sign($total_deduction + $cpf_employee, $user_id)
														?></td>
							...
							?>
									<tr>
										<td style="height: 25px;"><?php echo $deduction->name; ?></td>
							...
							?>
									<tr>
										<td style="height: 25px;"><?php echo $statutory_deduction->deduction_title; ?></td>
										<td class="blue-text" colspan="2"><?= $this->Xin_model->currency_sign($statutory_deduction->deduction_amount, $user_id) ?></td>
									</tr>
							<?php
								endforeach;
							endif; ?>
							<?php if ($leave_deduction): ?>
								<tr>
									<td>Leave Deductions</td>
									<td class="blue-text" colspan="2"><?= $this->Xin_model->currency_sign($leave_deduction, $user_id) ?></td>
								</tr>
							<?php endif; ?>
							<tr>
						...
						<tbody>
							...
							if ($overtimes):
								foreach ($overtimes as $overtime):
									$overtime_hours += $overtimes->overtime_hours;
									$overtime_hours_pay	+= $overtimes->overtime_rate;
								endforeach;
							endif;
							?>
							<tr>
								<td class="bg-second">Overtime Hours Worked</td>
								<td class="blue-text" colspan="2"><?= $overtime_hours ?></td>
							</tr>
							<tr>
								<td class="bg-second">Total Overtime Pay</td>
								<td class="blue-text"><?= $this->Xin_model->currency_sign($overtime_hours_pay, $user_id) ?></td>
								<td class="bg-second" style="width: 15%;text-align: center;">(E)</td>
							</tr>
							<?php
							if ($commissions):
								foreach ($commissions as $c):
									$name = $this->PaymentDeduction_Model->get_commissions_title($c->commission_id)
							?>
									<tr>
										<td style="height: 25px;"><?= $name[0]->payment_deduction_name ?></td>
							...
							if ($other_payments):
								foreach ($other_payments as $other_payment):
									$total_other_payment += $other_payment->payments_amount;
								endforeach;
							endif;
							if ($claims):
								foreach ($claims as $claim):
									$total_other_payment += $claim->amount;
								endforeach;
							endif;
							?>
							<tr>
								...
								<td class="blue-text"><?= $this->Xin_model->currency_sign($total_other_payment, $user_id)
														?></td>
							...
							if ($other_payments):
								foreach ($other_payments as $other_payment): ?>
									<tr>
										<td style="height: 25px;"><?= $other_payment->payments_title ?></td>
										<td class="blue-text" colspan="2"><?= $this->Xin_model->currency_sign($other_payment->payments_amount, $user_id) ?></td>
									</tr>
							<?php endforeach;
							endif;
							?>
							...
							if ($claims):
								foreach ($claims as $claim): ?>
									<tr>
										<td style="height: 25px;"><?= $claim->claim_type ?></td>
										<td class="blue-text" colspan="2"><?= $this->Xin_model->currency_sign($claim->amount, $user_id) ?></td>
									</tr>
							<?php endforeach;
							endif;
							?>
							...
							<tr>
								...
								<td colspan="2" class="blue-text" colspan="2"><?= $this->Xin_model->currency_sign($total_pay + $advance_pay, $user_id) ?></td>
							</tr>
							<tr>
