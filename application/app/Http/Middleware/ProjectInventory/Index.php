...
namespace App\Http\Middleware\ProjectInventory;
...
class Index
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if (request('noteresource_type') == 'project') {
            ...
            if (auth()->user()->is_team) {
                ...
                if ($this->projectpermissons->check('inventary-view', request('noteresource_id'))) {
                    return $next($request);
                }
            }
        }
    ...
    private function fronteEnd()
    {
        ...
        //add project notes
        if (request('noteresource_type') == 'project') {
            if (auth()->user()->role->role_inventary >= 2) {
                config([
