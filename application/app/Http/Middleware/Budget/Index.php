...
namespace App\Http\Middleware\Budget;
...
class Index
{
     ...
     *
     */
    public function __construct(ProjectInventoryPermissions $projectpermissons)
    {
    ...
    public function handle($request, Closure $next)
    {
        ...
        if (request('noteresource_type') == 'project') {
            ...
            if (auth()->user()->is_team) {
                ...
                if ($this->projectpermissons->check('budget-view', request('noteresource_id'))) {
                    return $next($request);
                }
            }
        }
    ...
    private function fronteEnd()
    {
        ...
        //add project notes
        if (request('noteresource_type') == 'project') {
            if (auth()->user()->role->role_purchase_requisiton >= 2) {
                config([
                    'visibility.list_page_actions_add_button' => true,
                ]);
            }
        }
        if (request('projectinventoryresource_type') == 'project') {
            config([
