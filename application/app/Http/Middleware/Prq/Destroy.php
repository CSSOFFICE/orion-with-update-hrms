...
class Destroy {
    ...
    public function handle($request, Closure $next) {
        ...
        if (!$note = DB::table('purchase_requistion')->where('purchase_requistion_id', $request->route('prq'))->first()) {
            Log::error("note could not be found", ['process' => '[permissions][notes][edit]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'note' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'purchase_requistion_id' => $request->route('prq')]);
