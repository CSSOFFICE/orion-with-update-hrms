...
namespace App\Http\Middleware\Variation;
...
class Show
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        //estimate id
        $bill_id = $request->route()->parameters();
        $bill_estimateid = $bill_id['estimate_id'];
        ...
        if ($bill_estimateid == '' || !$estimate = \App\Models\Variation::Where('vo_id', $bill_estimateid)->first()) {
            abort(404);
        }
    ...
    private function fronteEnd()
    {
        ...
        // print_r(request()->segment(5));die;

        //are we in editing mode
        if (request()->segment(5) == 'edit-estimate') {
            config([
