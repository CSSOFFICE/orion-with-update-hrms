...
class Index {
    ...
    public function handle($request, Closure $next) {
        ...
        if (request()->filled('addressresource_type') && request()->filled('addressresource_id')) {
            ...
            if (request('addressresource_type') == 'client') {
                request()->merge([
                    'filter_address_clientid' => request('addressresource_id'),
                ]);
            }
        }
        ...
        if (auth()->user()->is_client) {
            if (auth()->user()->role->role_billing_address >= 1) {
        ...
        //permission denied
        Log::error("permission denied", ['process' => '[permissions][address][index]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
        abort(403);
    }
    ...
    private function fronteEnd() {
         ...
         * */
        if (request('addressresource_type') != '' || is_numeric(request('addressresource_id'))) {
            request()->merge([
                'resource_query' => 'ref=list&addressresource_type=' . request('addressresource_type') . '&addressresource_id=' . request('addressresource_id'),
            ]);
        } else {
            request()->merge([
