...
class Destroy {
    ...
    public function handle($request, Closure $next) {
        ...
        if (is_numeric($request->route('address'))) {
            $ids[$request->route('address')] = 'on';
            request()->merge([
                'ids' => $ids,
            ]);
        }
        ...
        if (is_array(request('ids'))) {
            ...
            foreach (request('ids') as $id => $value) {
                ...
                if ($value == 'on') {
                    //validate
                    if (!$contact = \App\Models\billing_address::Where('id', $id)->first()) {
                    ...
                    if (auth()->user()->is_client) {
                        if ($contact->clientid != auth()->user()->clientid) {
                            Log::error("permission denied", ['process' => '[permissions][address]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'user_id' => auth()->id()]);
                            abort(403);
                        }
                    }
                }
            ...
            if (auth()->user()->is_team) {
                if (auth()->user()->role->role_billing_address < 3) {
                    Log::error("permission denied", ['process' => '[permissions][address]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'user_id' => auth()->id()]);
            ...
            Log::error("no items were sent with this request", ['process' => '[permissions][address]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'contact id' => $contact_id ?? '']);
            abort(409);
        }
