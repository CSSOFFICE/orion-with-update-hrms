...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the variation
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/variation/components/table/ajax';
                $dom_container = '#variation-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/variation/components/table/table';
                $dom_container = '#variation-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/variation/tabswrapper';
            ...
            if ($variation->currentPage() < $variation->lastPage()) {
                $url = loadMoreButtonUrl($variation->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'variation', 'stats', 'categories', 'tags', 'projects'))->render();
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('lang.variation')),
            ];
            ...
            $jsondata['dom_classes'][] = [
                'selector' => '#tabs-menu-billing, #tabs-menu-variation',
                'action' => 'add',
                'value' => 'active',
            ];
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($variation->currentPage() + 1, request('source'));
            $page['loading_target'] = 'temp-td-container';
            $page['visibility_show_load_more'] = ($variation->currentPage() < $variation->lastPage()) ? true : false;
            return view('pages/variation/wrapper', compact('page', 'variation', 'stats', 'categories', 'tags', 'projects','quotation_templates'))->render();
        }
