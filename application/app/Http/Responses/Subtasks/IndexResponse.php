...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/subtasks/components/table/ajax';
                $dom_container = '#subtasks-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/subtasks/components/table/wrapper';
                $dom_container = '#subtasks-table-wrapper';
                $dom_action = 'replace';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/subtasks/tabswrapper';
            ...
            if ($subtasks->currentPage() < $subtasks->lastPage()) {
                $url = loadMoreButtonUrl($subtasks->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'subtasks'))->render();
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('lang.subtasks')),
            ];
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($subtasks->currentPage() + 1, request('source'));
            $page['loading_target'] = 'subtasks-td-container';
            $page['visibility_show_load_more'] = ($subtasks->currentPage() < $milestones->lastPage()) ? true : false;
            return view('pages/subtasks/wrapper', compact('page', 'subtasks'))->render();
        }
