...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/address/components/table/ajax';
                $dom_container = '#address-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/address/components/table/table';
                $dom_container = '#address-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/address/tabswrapper';
            ...
            if ($address->currentPage() < $address->lastPage()) {
                $url = loadMoreButtonUrl($address->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'address'))->render();
            ...
            $page['loading_target'] = 'address-td-container';
            $page['visibility_show_load_more'] = ($address->currentPage() < $address->lastPage()) ? true : false;
            return view('pages/address/wrapper', compact('page', 'address'))->render();
        }
    }
