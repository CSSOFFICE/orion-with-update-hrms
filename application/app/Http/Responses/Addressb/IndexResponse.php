...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/addressb/components/table/ajax';
                $dom_container = '#addressb-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/addressb/components/table/table';
                $dom_container = '#addressb-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/addressb/tabswrapper';
            ...
            if ($addressb->currentPage() < $addressb->lastPage()) {
                $url = loadMoreButtonUrl($addressb->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'addressb'))->render();
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($address->currentPage() + 1, request('source'));
            $page['loading_target'] = 'addressb-td-container';
            $page['visibility_show_load_more'] = ($addressb->currentPage() < $addressb->lastPage()) ? true : false;
            return view('pages/addressb/wrapper', compact('page', 'addressb'))->render();
        }
    }
