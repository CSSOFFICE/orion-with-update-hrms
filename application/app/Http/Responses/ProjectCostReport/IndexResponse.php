...
namespace App\Http\Responses\ProjectCostReport;
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/reports/expense/components/table/ajax';
            ...
            case 'search':
                $template = 'pages/reports/expense/components/table/table';
            ...
            default:
                $template = 'pages/reports/expense/tabswrapper';
            ...
            if ($invoice->currentPage() < $invoice->lastPage()) {
                $url = loadMoreButtonUrl($invoice->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'invoice','invoice_items','employee_salary','employee_total_salary'))->render();
            ...
            $jsondata['dom_classes'][] = [
                'selector' => '#tabs-menu-report, #tabs-menu-expenses',
                'action' => 'add',
                'value' => 'active',
            ];
            ...
            $html = view('misc/list-pages-stats')->render();
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($profitloss->currentPage() + 1, request('source'));
            $page['loading_target'] = 'expenses-td-container';
            $page['visibility_show_load_more'] = ($profitloss->currentPage() < $profitloss->lastPage()) ? true : false;
           
            return view('pages/reports/expense/wrapper', compact('page', 'invoice','invoice_items','employee_salary','employee_total_salary'))->render();
        }
