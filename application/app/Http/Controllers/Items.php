...
class Items extends Controller {
    ...
    public function Getproductbyid(){
        echo json_encode(DB::table('product')->where('product_id',request('id'))->get()->first());
    }
    ...
    public function onchange_function(){
        ...
        // ->select('purchase_order.*', DB::raw('SUM(purchase_order_item_mapping.prd_total) as total'))
        // ->where('purchase_order.purchase_order_id', '=', $order_num)
        // ->groupBy('purchase_order.purchase_order_id')
        // ->get();
        // select("purchase_order.*,SUM(purchase_order_item_mapping.prd_total) as total")->from("purchase_order")
        //                         ->join('purchase_order_item_mapping','purchase_order.purchase_order_id=porder_id')->where('porder_id',$order_num)->first();
...
// if ($data->isNotEmpty()) {
//     // Access the total from the first result (assuming there's only one result due to grouping)
//     $total = $data->first()->total;
//     // Use $total as needed
//     dd($total);
// } else {
//     // Handle the case where no results were found
//     dd('No results found for the given order number.');
// }
        // print_r($data);
    }
}
