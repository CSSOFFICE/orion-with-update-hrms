...
use App\Http\Requests\Quo\TaskStoreUpdate;
use App\Http\Responses\Quo\AttachFilesResponse;
use App\Http\Responses\Quo\ChecklistResponse;
use App\Http\Responses\Quo\CreateResponse;
use App\Http\Responses\Quo\DestroyResponse;
use App\Http\Responses\Quo\IndexKanbanResponse;
use App\Http\Responses\Quo\IndexListResponse;
use App\Http\Responses\Quo\ShowResponse;
use App\Http\Responses\Quo\StoreChecklistResponse;
use App\Http\Responses\Quo\StoreCommentResponse;
use App\Http\Responses\Quo\StoreResponse;
use App\Http\Responses\Quo\TimerStartResponse;
use App\Http\Responses\Quo\TimerStopResponse;
use App\Http\Responses\Quo\UpdateChecklistResponse;
use App\Http\Responses\Quo\UpdateErrorResponse;
use App\Http\Responses\Quo\UpdateResponse;
use App\Http\Responses\Quo\UpdateStatusResponse;
...
use App\Permissions\QuoPermissions;
...
use App\Repositories\QuoAssignedRepository;
use App\Repositories\QuoRepository;
...
class quo extends Controller
{
    ...
    public function __construct(
        QuoRepository $taskrepo,
        EstimateRepository $estimaterepo,
        TagRepository $tagrepo,
        UserRepository $userrepo,
        TimerRepository $timerrepo,
        QuoPermissions $taskpermissions,
        ...
        //route middleware
        $this->middleware('quoMiddlewareTimer')->only([
            'timerStart',
            'timerStop',
            'timerStopAll',
        ]);
        ...
        $this->middleware('quoMiddlewareIndex')->only([
            'index',
            'update',
            'toggleStatus',
            'store',
        ...
        $this->middleware('quoMiddlewareCreate')->only([
            'create',
            'store',
        ]);

        $this->middleware('quoMiddlewareShow')->only([
            'show',
        ]);

        $this->middleware('quoMiddlewareEdit')->only([
            'updateDescription',
            'updateTitle',
            'updateStartDate',
            'updateDueDate',
        ...
        $this->middleware('quoMiddlewareParticipate')->only([
            'storeComment',
            'attachFiles',
        ]);

        $this->middleware('quoMiddlewareDeleteAttachment')->only([
            'deleteAttachment',
        ]);

        $this->middleware('quoMiddlewareDownloadAttachment')->only([
            'downloadAttachment',
        ]);

        $this->middleware('quoMiddlewareDeleteComment')->only([
            'deleteComment',
        ]);

        $this->middleware('quoMiddlewareEditDeleteChecklist')->only([
            'updateChecklist',
            'deleteChecklist',
            'toggleChecklistStatus',
        ]);

        $this->middleware('quoMiddlewareDestroy')->only([
            'destroy',
        ]);

        $this->middleware('quoMiddlewareAssign')->only([
            'updateAssigned',
        ]);
    }
    /**
    ...
    public function GetDefaultAddress()
    {
        if (request('add')) {
            ...
            echo json_encode(DB::table('billing_addresses')->where('client_id', request('id'))->get());
        } else {

            echo json_encode(DB::table('billing_addresses')->where('client_id', request('id'))->where('d_address', 1)->get()->first());
        }
    }
    public function Defaoutdataget()
    {

        echo json_encode(DB::table('billing_addresses')->where('id', request('id'))->get()->first());
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function store(TaskStoreUpdate $request, QuoAssignedRepository $assignedrepo)
    {
       
        // die;

        //get client id of attached project (if this is a project task)
        // $project = \App\Models\Project::find(request('bill_clientid'));
    ...
    private function pageSettings($section = '', $data = [])
    {
        ...
        $page = [
            ...
            'dynamic_search_url' => url('quo/search?action=search&taskresource_id=' . request('taskresource_id') . '&taskresource_type=' . request('taskresource_type')),
            'add_button_classes' => '',
            'load_more_button_route' => 'quo',
            'source' => 'list',
        ];

        //default modal settings (modify for sepecif sections)
        $page += [
            'add_modal_title' => "Quotation",
            'add_modal_create_url' => url('quo/create?taskresource_id=' . request('taskresource_id') . '&taskresource_type=' . request('taskresource_type')),
            'add_modal_action_url' => url('quo?taskresource_id=' . request('taskresource_id') . '&taskresource_type=' . request('taskresource_type') . '&count=' . ($data['count'] ?? '')),
    ...
    private function statsWidget($data = array())
    {
        ...
        // $count_in_progress = $this->taskrepo->search('', ['stats' => 'Draft']);

        // $count_testing = $this->taskrepo->search('', ['stats' => 'accepted']);
        // $count_awaiting_feedback = $this->taskrepo->search('', ['stats' => 'Expired']);
        // $count_completed = $this->taskrepo->search('', ['stats' => 'declined']);
