...
class Projects extends Controller
{
    ...
    public function store(
        ...
        $assigned_usersuu = $assignedrepo->addSite($project_id, '');
    ...
    public function update(ProjectValidation $request, ProjectAssignedRepository $assignedrepo, ProjectManagerRepository $managerrepo, $id)
    {
        ...
        $project = $projects->first();
        $assigned_usersuu = $assignedrepo->updateSite($id);
        ...
        foreach (request('assigned') as $keyy => $user_idid) {
            foreach ($user_idid as $key => $user_id) {
                $assigned_users = $assignedrepo->add($id, $user_id, $keyy);
    ...
    public function Delete_Project_w()
    {
        DB::table('purchase_order')->where('project_id', request('project_id'))->delete();
        DB::table('purchase_requistion')->where('project_id', request('project_id'))->delete();
        DB::table('grn_item_mapping')->where('prd_id', request('project_id'))->delete();
        DB::table('stock_move_log')->where('prj_id', request('project_id'))->delete();
    }
    ...
    /**
     * Remove the specified project from storage.f
    ...
    public function changeStatusUpdate()
    {
        ...
        if (request('project_status') == 'completed') {
            try {
                DB::table('warehouse')->where('project_id', $project->project_id)->delete();
            } catch (\Exception $e) {
            }
        }
    ...
    private function statsWidget($data = array())
    {
        ...
        $count_delay = $this->projectrepo->search('', ['stats' => 'count-delay']);
