...
use App\Http\Responses\Invoices\DestroyInvoiceResponse;
...
use App\Http\Responses\Invoices\UpdateFinanceResponse;
...
class Invoices extends Controller
{
    ...
    public function index(ProjectRepository $projectrepo, CategoryRepository $categoryrepo)
    {
        ...
        $id = request('invoiceresource_id');
        $invoices = $this->invoicerepo->get_invoices($id);
        //    print_r($invoices);die;

        $payment_terms = $this->invoicerepo->get_payment_term();
        //dd(DB::getQueryLog());
    ...
    public function create()
    {
        ...
        $payment_terms = $this->invoicerepo->get_payment_term();
        $in = $this->invoicerepo->search();
        $budgtrepo = $this->budgtrepo->search($id);
        $projects = $this->projectrepo->search($id);
        $project = $projects->first();
        // print_r($project);
        // die;
        $templete_category = $this->budgtrepo->get_category_data(request('invoiceresource_id'));


        $gst = $this->invoicerepo->get_gst();
        $client = $this->invoicerepo->get_client($id);
        $quotation_amount = $this->invoicerepo->ajax_project_quotation_info($id);
        $total_invoice_amount = $this->invoicerepo->ajax_invoice_info($id);
...
// echo "pp";die;
        ...
        $total_invoice = floatval(request('total_amount')) + floatval(request('total_invoice_amount'));

        if ($total_invoice > floatval(request('quotation_amount'))) {
            // abort(409, __('lang.error_quotation_exceed'));
        }
        //create the invoice
    ...
    public function show($id)
    {
        ...
        $payload =
            [
                'page' => $this->pageSettings('invoice', $payload['bill']),
                'payment_terms' => $payment_terms,
                'gst'  => $gst,
                //  'client'=>$client
            ];
    ...
    public function edit($id)
    {
        ...
        $invoice = $this->invoicerepo->get_invoice_by_id($id);


        $invoice_item = $this->invoicerepo->get_invoice_items($id);
        $project_id = request('invoiceresource_id');
        $budgtrepo = $this->budgtrepo->search(request('invoiceresource_id'));
        ...
        $payment_terms = $this->invoicerepo->get_payment_term();
        $quotation_amount = $this->invoicerepo->ajax_project_quotation_info($project_id);
        $total_invoice_amount = $this->invoicerepo->ajax_invoice_info($project_id);
        $gst = $this->invoicerepo->get_gst();
        $projects = $this->projectrepo->search(request('invoiceresource_id'));
        $project = $projects->first();
        //not found
        // var_dump($invoice[0]->gst);die;
        // print_r($budgtrepo);die;
        // $templete_category = $this->budgtrepo->get_category_data(request('invoiceresource_id'));
        $get_milestone = $this->milestonerepo->get_milestone(request('invoiceresource_id'));
    ...
    public function update($id)
    {
        ...
        $total_invoice = floatval(request('u_total_amount')) + floatval(request('total_invoice_amount'));
        ...
        //get project
        $invoices = $this->invoicerepo->get_invoice_by_id($id);
        ...
        //generate a response
        return new UpdateFinanceResponse($payload);
    }
    ...
    public function destroy(DestroyRepository $destroyrepo)
    {
        ...
        foreach (request('ids') as $id => $value) {
            ...
            if ($value == 'on') {
                ...
                //add to array
                $allrows[] = $id;
                FinanceInvoiceMapping::Where('invoice_id', $id)->delete();
                FinanceInvoice::Where('invoice_id', $id)->delete();
            }
        }
        //reponse payload
        $payload = [
    ...
    //     //process reponse
    //     return new DestroyInvoiceResponse($payload);
    // }
    /**
    ...
    private function statsWidget($data = array())
    {
        ...
        // print_R($sum_overdue_balances.'ll');die;
