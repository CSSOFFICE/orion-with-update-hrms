...
class Expenses extends Controller
{
    ...
    public function index(ProjectRepository $projectrepo, CategoryRepository $categoryrepo)
    {
        ...
        $expenses_data = $this->expenserepo->get_expense();
    ...
    public function create(CategoryRepository $categoryrepo)
    {
        ...
        $get_milestone = $this->milestonerepo->get_milestone(request('expenseresource_id'));
        $category_quotation = $this->BudgetRepository->get_category_data(request('expenseresource_id'));
        // print_r($get_milestone);die;
        $budgtrepo = $this->BudgetRepository->search(request('expenseresource_id'));
        //expense categories
        $categories = $categoryrepo->get('expense');
        ...
        $payload = [
            'page' => $page,
            'categories' => $categories,
            'quotation_category' => $get_milestone,
            'budgtrepo' => $budgtrepo,
        ];
    ...
    //         ->leftJoin('expenses', 'expenses.purchase_order_id', '=', 'xin_payable.purchase_order_id')
    //         ->on('expenses.purchase_order_id', '=', 'xin_payable.purchase_order_id')
    //         ->where('expenses.expense_id', $id)
    //         ->get();
    //     // First, get the expense to check the condition
    //     // $expense = DB::table('expenses')->where('expense_id', $id)->get();

    //     // if ($expense) {
    //     //     // Now check if there is any payable record with that purchase_order_id
    //     //     $xin_payable_query = DB::table('xin_payable');

    //     //     if ($expense->purchase_order_id != 0) {
    //     //         $xin_payable_total = $xin_payable_query
    //     //             ->Join('expenses', 'expenses.purchase_order_id', '=', 'xin_payable.purchase_order_id')
    //     //             ->where('expenses.expense_id', $id)
    //     //             ->get();
    //     //     } else {
    //     //         $xin_payable_total = $xin_payable_query
    //     //             ->join('expenses', 'expenses.purchase_order_no', '=', 'xin_payable.manual_po_number')
    //     //             ->where('expenses.expense_id', $id)
    //     //             ->get();
    //     //     }
    //     // }
    ...
    //         'xin_payable_total' => $xin_payable_total,
    //     ];
    ...
    {
        // echo "abcd";die;
        // Get the expense (collection of one item)
        $expenses = $this->expenserepo->search($id);
        // print_r($expenses);die;
        // Get the first expense record
        $expense = $expenses->first();

        // Initialize payable result
        $xin_payable_total = collect(); // default to empty collection

        // Safety check
        if ($expense) {
            if ($expense->purchase_order_id != 0) {
                // Case: use purchase_order_id
                $xin_payable_total = DB::table('xin_payable')
                    ->join('expenses', 'expenses.purchase_order_id', '=', 'xin_payable.purchase_order_id')
                    ->where('expenses.expense_id', $id)
                    ->select('xin_payable.*')
                    ->get();
            } elseif (!empty($expense->manual_po_number)) {
                // Case: use manual_po_number
                $xin_payable_total = DB::table('xin_payable')
                    ->join('expenses', 'expenses.purchase_order_no', '=', 'xin_payable.manual_po_number')
                    ->where('expenses.expense_id', $id)
                    ->select('xin_payable.*')
    ...
    {
        $xin_payable_total = DB::table('expenses')
            // ->join('expenses', 'expenses.purchase_order_id', '=', 'xin_payable.purchase_order_id')
