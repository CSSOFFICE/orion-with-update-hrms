...
class Payments extends Controller
{
    ...
    public function create()
    {
        ...
        $invoices_li = \DB::table('finance_invoice')->where('project_id', request('paymentresource_id'))->get();

        //reponse payload
        $payload = [
            'page' => $this->pageSettings('create'),
            'invoice' => $invoice,
            'invoice_li' => $invoices_li,
        ];
    ...
    public function store(PaymentStoreUpdate $request, InvoiceRepository $invoicerepo)
    {
        ...
        // }
        $invoice = FinanceInvoice::where('invoice_id',request('payment_invoiceid'))->first();


        //additional information
        request()->merge([
            'payment_clientid' =>$invoice->client_id,
            'payment_projectid' =>$invoice->project_id,
