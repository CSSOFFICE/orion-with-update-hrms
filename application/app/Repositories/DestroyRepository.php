...
class DestroyRepository {
    ...
    public function destroyProject($project_id) {
        ...
        if (!is_numeric($project_id)) {
            Log::error("validation error - invalid params", ['process' => '[DestroyRepository]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }

        //get project and validate
        if (!$project = \App\Models\Project::Where('project_id', $project_id)->first()) {
            // Log::error("record could not be found", ['process' => '[DestroyRepository]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }
    ...
    public function destroyEstimate($estimate_id) {
        ...
        $dd=DB::table('quos')->where(['task_id'=>$estimate_id])->delete();
        $dd=DB::table('quotation_templates')->where(['estimates_id'=>$estimate_id])->delete();
