...
class PaymentRepository
{
    ...
    public function search($id = '', $data = array())
    {
        ...
        $payments->leftJoin('finance_invoice', 'finance_invoice.invoice_id', '=', 'payments.payment_invoiceid');
        ...
        if ($data['apply_filters']) {
            ...
            if (request()->filled('filter_payment_date_start')) {
                $payments->where('payment_date', '>=', request('filter_payment_date_start'));
            }

            //filter: date (end)
            if (request()->filled('filter_payment_date_end')) {
                $payments->where('payment_date', '<=', request('filter_payment_date_end'));
            }
