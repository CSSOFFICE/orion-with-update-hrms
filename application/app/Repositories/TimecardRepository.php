...
class TimecardRepository
{
     ...
     * Inject dependecies
     */
    public function __construct(Timecard $xinem, xin_employees $timer)
    {
        $this->timer = $timer;
        $this->xinem = $xinem;
    }
    ...
    public function geemployee()
    {
        if (request()->filled('search_query') || request()->filled('query')) {
            $dd = DB::table('xin_employees')->where('first_name', 'LIKE', '%' . request('search_query') . '%')->get();
        } else {

            // $dd = DB::table('xin_employees')->get();
            $iid=request('timecardresource_id')??request('timesheetresource_id');
            $dd = DB::table('xin_employees')
                ->select('xin_employees.*')
                ->join('projects_assigned', 'xin_employees.user_id', '=', 'projects_assigned.projectsassigned_userid')
                ->where('projects_assigned.projectsassigned_projectid',$iid)
                ->get();
        }

        return $dd;
    }
    public function getEmploeeHours()
    {

        $a = DB::table('timecards')->where('status', 1)->where('date', date('m-Y'))->get();
