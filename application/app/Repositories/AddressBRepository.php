...
class AddressBRepository {
     ...
     * Inject dependecies
     */
    public function __construct(shipping_address $shipping_address) {
    ...
    public function get($id = '') {
        ...
        //new query
        $shipping_address = $this->shipping_address->newQuery();
        ...
        if (request()->filled('clientid')) {
            $shipping_address->where('client_id', request()->input('clientid'));
        }

        return $shipping_address->first();
    }
    ...
    public function exists($id = '') {
        ...
        //check
        $shipping_address->where('id', '=', $id);
        return $shipping_address->exists();
    }
    ...
    public function search($id = '') {
        ...
        // $shipping_address->leftjoinClients();

        //ignore system user
        // $shipping_address->where('id', '>', 0);

        //filter: type
        // if (request()->filled('type')) {
        //     $shipping_address->where('type', request('type'));
        // }

        //filter: status
        // if (request()->filled('status')) {
        //     $shipping_address->where('status', request('status'));
        // }

        //filter: id
        if (request()->filled('id')) {
            $shipping_address->where('id', request('id'));
        }
        if (is_numeric($id)) {
            $shipping_address->where('id', $id);
        }
        ...
        //     $shipping_address->where('created', '>=', request('filter_date_created_start'));
        // }

        // //filter: created date (end)
        // if (request()->filled('filter_date_created_end')) {
        //     $shipping_address->where('created', '<=', request('filter_date_created_end'));
        // }

        //filters: primary or not
        // if (request()->filled('filter_account_owner')) {
        //     $shipping_address->where('account_owner', request('filter_account_owner'));
        // }
        ...
        //     $shipping_address->whereIn('id', request('filter_name'));
        // }
        ...
        //     $shipping_address->whereIn('id', request('filter_email'));
        // }
        ...
        //     $shipping_address->whereIn('clientid', request('filter_clientid'));
        // }

        //sanity: client
        if (request()->filled('clientid')) {
            $shipping_address->where('client_id', request('clientid'));
        }

        //resource filtering
        if (request()->filled('addressbresource_type') && request()->filled('addressbresource_id')) {
            switch (request('addressbresource_type')) {
            case 'client':
                $shipping_address->where('client_id', request('addressbresource_id'));
                break;
            }
        }
        ...
        //     $shipping_address->where(function ($query) {
        ...
        //     if (Schema::hasColumn('shipping_address', request('orderby'))) {
        //         $shipping_address->orderBy(request('orderby'), request('sortorder'));
        //     }
        //     //others
        //     switch (request('orderby')) {
        //     case 'company_name':
        //         $shipping_address->orderBy('client_company_name', request('sortorder'));
        //         break;
        //     }
        // } else {
        //     //default sorting
        //     $shipping_address->orderBy('shipping_address.first_name', 'asc');
        // }

        //eager load
        $shipping_address->with([
            'role',
        ]);

        // Get the results and return them.
        return $shipping_address->paginate(config('system.settings_system_pagination_limits'));
    }
    ...
    public function updatePreferences($id = '') {
        ...
        if ($shipping_address = $this->shipping_address->find($id)) {
    ...
    public function create() {
        ...
        //save new user
        $user = new $this->shipping_address;

        //data
        $user->client_id = request('addressbresource_id');
    ...
    public function update($id) {
        ...
        //get the user
        if (!$user = $this->shipping_address->find($id)) {
            Log::error("record could not be found", ['process' => '[AddresbsRepository]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'user_id' => $id ?? '']);
            return false;
        }
