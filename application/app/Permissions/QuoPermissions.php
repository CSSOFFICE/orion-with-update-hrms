...
class QuoPermissions {
    ...
    public function check($action = '', $task = '') {
        ...
        if ($task instanceof \App\Models\Task || $task instanceof \Illuminate\Pagination\LengthAwarePaginator) {
            ...
            return false;
            // echo "<pre>";print_r($assigned_project_users);exit;
            // echo "<pre>";print_r($project->assigned->pluck('id'));exit;
        } else {
            Log::error("the task coud not be found", ['process' => '[permissions][task]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }
        ...
        //failed
        Log::error("permissions denied on this task", ['process' => '[permissions][tasks]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
        return false;
    }
