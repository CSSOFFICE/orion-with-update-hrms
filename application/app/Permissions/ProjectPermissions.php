    ...
    public function check($action = '', $project = '')
    {
        ...
        if ($action == 'users') {
            ...
            foreach ($users as $user) {
                if ($user->id > 0) {
                    ...
                    if ($user->type == 'team') {
                        // gloabl user
                        if ($user->role->role_projects ?? 0 >= 1 && $user->role->role_projects_scope == 'global') {
                            $list[] = $user->id;
                    ...
                    //client
                    // echo "<pre>";
                    // print_r($project->client->client_id);die;
                    if ($user->type == 'client') {
        ...
        if ($action == 'vo-view') {
            ...
            if (auth()->user()->is_team) {
                ...
                if (auth()->user()->role->role_variation_order >= 1) {
                    //global
                    if (auth()->user()->role->role_variation_order_scope == 'global') {
                        return true;
                    }
                    //managers
                    if ($project_managers->contains(auth()->id())) {
        ...
        if ($action == 'prq-view') {
            ...
            //team
            if (auth()->user()->is_team) {
                if (auth()->user()->role->role_purchase_requisiton >= 1) {
                    //global
                    if (auth()->user()->role->role_purchase_requisiton_scope == 'global') {
                        return true;
                    }
                    //managers
                    if ($project_managers->contains(auth()->id())) {
        ...
        if ($action == 'inventary-view') {

            //team
            if (auth()->user()->is_team) {
                if (auth()->user()->role->role_inventary >= 1) {
                    //global
                    if (auth()->user()->role->role_inventary_scope == 'global') {
                        return true;
                    }
                    //managers
                    if ($project_managers->contains(auth()->id())) {
        ...
        if ($action == 'budget-view') {

            //team
            if (auth()->user()->is_team) {
                if (auth()->user()->role->role_budget >= 1) {
                    //global
                    if (auth()->user()->role->role_budget_scope == 'global') {
                        return true;
                    }
                    //managers
                    if ($project_managers->contains(auth()->id())) {
