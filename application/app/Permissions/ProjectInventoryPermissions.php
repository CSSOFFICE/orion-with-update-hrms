...
class ProjectInventoryPermissions
{
    ...
    public function check($action = '', $note = '')
    {
        ...
        //GET THE RESOURCE
        if (is_numeric($note)) {
            if (!$note = DB::table('purchase_requistion')->where('purchase_requistion_id', $note)->first()) {
        ...
        if (is_object($note) && isset($note->purchase_requistion_id)) {
            //all is ok
        } else {
            Log::error("the note coud not be found", ['process' => '[permissions][note]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        ...
        if ($action == 'view') {
            ...
            if ($note->note_visibility == 'public') {
                ...
                if ($note->noteresource_type == 'project') {
                    ...
                    if ($this->projectpermissons->check('notes-view', $note->project_id)) {
                        return true;
                    }
                }
        ...
        if ($action == 'edit-delete') {
            ...
            //project note
            if ($note->noteresource_type == 'project') {
                if ($this->projectpermissons->check('super-user', $note->project_id)) {
                    return true;
                }
            }
            //client note
        ...
        //failed
        Log::error("permissions denied on this note", ['process' => '[permissions][files]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
        return false;
    }
}
