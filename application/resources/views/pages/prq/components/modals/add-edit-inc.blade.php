                ...
                <table style="width: 100%;">
                    ...
                    <tr>
                        <td style="width: 33.33%;text-align:left;border:1px solid;" colspan="2"><b>Site:</b>
                            <input type="text" name="site_address" id="site_address" class="form-control" value="{{$budgtrepo[0]->site_address??''}}">
                        </td>
                    ...
                    <tr>
                        <td style="width:70%;text-align:left;border:1px solid;">
                            ...
                            <select class="form-control" name="supervisor" id="supervisor">
                                ...
                                @foreach ($dataSupervisor as $customer)
                                <option value='{{$customer->user_id }}' {{($customer->user_id==$project_d[0]->Supervisor)?"selected":"" }}> {{$customer->first_name }}{{$customer->last_name }}</option>
                                @endforeach
                            </select>
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#milestone_id").on('change', function() {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ url('admin/purchase/get_tasks_by_milestone') }}" + $('#milestone_id').val(),
                    dataType: "json",
        ...
        function getProductDetail(id, number) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ url('admin/purchase/get_product_details/')}}" + id,
                data: JSON,
            ...
            <div class="form-body">
                ...
                <div class="invoice">
                    <div class="left">
                        ...
                        <input type="hidden" name="project_id" value="{{$budgtrepo[0]->project_id??''}}">
                        <select class="form-control" name="milestone_id" id="category_id_prili">
                            <option>Choose Category</option>
                            <?php foreach ($templete_category as $k => $purchase) { ?>
                                <option value="<?php echo $purchase->milestonecategory_id; ?>" {{$purchase->milestonecategory_id==$note->mile_stone?'selected':''}}><?php echo $purchase->milestonecategory_title; ?></option>
                            <?php } ?>
                        </select>
                        <label>Task</label>
                        <select name="task_id" id="task_id" class="form-control" data-plugin="select_hrm">
                            @foreach($budgtrepo as $tas)
                            @if($tas->task_cat_id==$note->mile_stone)
                            <option value="<?php echo $tas->task_id; ?>" {{($tas->task_id==$note->task)?'selected':''}}><?php echo $tas->task_title; ?></option>
                            @endif
                            @endforeach
                    ...
                    <table style="width: 100%;">
                        ...
                        <tr>
                            <td style="width: 33.33%;text-align:left;border:1px solid;" colspan="2"><b>Site:</b>
                                <input type="text" name="site_address" id="site_address" class="form-control" value="{{$note->site_address??''}}">
                            </td>
                    ...
                    <table>
                        <tr>
                            <td style="width:70%;text-align:left;border:1px solid;">
                                ...
                                <select class="form-control" name="supervisor" id="supervisor">
                                    <option>Select Supervisor</option>
                                    @foreach ($dataSupervisor as $customer)
                                    <option value="{{ $customer->user_id }}" {{($customer->user_id==$project_d[0]->Supervisor)?"selected":"" }}>
                                ...
                                <select class="form-control" name="sub_contractor">
                                    ...
                                    <option value="{{ $customer->user_id }}" {{ $customer->user_id == $note->sub_contractor ? 'selected' : '' }}>
                                        {{ $customer->first_name }} {{ $customer->last_name }}
                                    </option>
                                    @endforeach
                                </select>
                        ...
                        <tr>
                            ...
                            <td style="width:30%;text-align:left;border:1px solid;">
                                <label>Date of Materials required: </label><br />
                                <p>Earliest Date:<input type="date" value="{{$note->earliest_date}}" name="earliest_date" class="date form-control"></p><br />
                                <p>Latest Date:<input type="date" name="latest_date" value="{{$note->latest_date}}" class="date form-control"></p>
                                <br />
                            </td>
