...
@php
$alluoms = DB::table('xin_unit')->get();
@endphp
<div class="col-12" id="content-container">
    <form id="templates-form" method="POST" action="{{ url('/estimates/' . $bill->bill_estimateid . '/edit-estimate') }}">
        @csrf
        <div id="template-summary" style="overflow-x:scroll;" class="template-content">
            @include('pages.bill.components.elements.templates.summary')
        </div>
        <div id="template-preliminaries" class="template-content" style="display: none;overflow-x:scroll;">
            @include('pages.bill.components.elements.templates.preliminaries')
        </div>
        <div id="template-insurance" class="template-content" style="display: none;overflow-x:scroll;">
            @include('pages.bill.components.elements.templates.insurance')
        </div>
        <div id="template-schedule_of_works" class="template-content" style="display: none;overflow-x:scroll;">
            @include('pages.bill.components.elements.templates.schedule_of_works')
        </div>
        <div id="template-plumbing_sanity" class="template-content" style="display: none;overflow-x:scroll;">
            @include('pages.bill.components.elements.templates.plumbing_sanity')
        </div>
        <div id="template-elec_acme" class="template-content" style="display: none;overflow-x:scroll;">
            @include('pages.bill.components.elements.templates.elec_acme')
        </div>

        <div id="template-external_works" class="template-content" style="display: none;overflow-x:scroll;">
            @include('pages.bill.components.elements.templates.external_works')
        </div>
        <div id="template-pc_ps_sums" class="template-content" style="display: none;overflow-x:scroll;">
            @include('pages.bill.components.elements.templates.pc_ps_sums')
        </div>
        <div id="template-others" class="template-content" style="display: none;overflow-x:scroll;">
            @include('pages.bill.components.elements.templates.others')
        </div>
...
<script>
    ...
    $(document).ready(function() {
        $('.unit-input').css('width', 'fit-content');
    ...
    $('body').on('input', '.humara-class', function() {
        ...
        let z = $sktr.find('.qty-input').val() || 0;
        ...
        let d_value = (d / 100);
        let datae = (d_value * (b + c)).toFixed(2);

        $sktr.find('.e').val(datae);
        let e = parseFloat($sktr.find('.e').val()) || 0
        let f = parseFloat($sktr.find('.f').val()) || 0;
        let net_rate = (a + b) + (c + e + f) || 0;
        $sktr.find('.g').val(net_rate.toFixed(2));
        console.log('Total: ', net_rate.toFixed(2));
        ...
        let k = parseFloat($sktr.find('.k').val(z * dataj)) || 0;
        // console.log('K: ');
