<div class="card count-{{ @count($addressb) }}" id="addressb-table-wrapper">
    <div class="card-body">
        <div class="table-responsive list-table-wrapper">
            ...
            <table id="contacts-list-table" class="table m-t-0 m-b-0 table-hover no-wrap contact-list"
                ...
                <thead>
                    <tr>
                        ...
                        <th class="contacts_col_first_name"><a class="js-ajax-ux-request js-list-sorting"
                                ...
                                data-url="{{ urlResource('/address?action=sort&orderby=first_name&sortorder=asc') }}">{{ "ID" }}<span class="sorting-icons"><i class="ti-arrows-vertical"></i></span></a>
                        </th>

                        <th class="contacts_col_company"><a class="js-ajax-ux-request js-list-sorting"
                                id="sort_company_name" href="javascript:void(0)"
                                data-url="{{ urlResource('/address?action=sort&orderby=company_name&sortorder=asc') }}">{{ "Address" }}<span class="sorting-icons"><i class="ti-arrows-vertical"></i></span>
                        </th>

                        <th class="contacts_col_email"><a class="js-ajax-ux-request js-list-sorting" id="sort_email"
                                href="javascript:void(0)"
                                data-url="{{ urlResource('/address?action=sort&orderby=email&sortorder=asc') }}">{{ "Unit Number" }}<span class="sorting-icons"><i class="ti-arrows-vertical"></i></span></th>
                        <th class="contacts_col_phone"><a class="js-ajax-ux-request js-list-sorting" id="sort_phone"
                                href="javascript:void(0)"
                                data-url="{{ urlResource('/address?action=sort&orderby=phone&sortorder=asc') }}">{{ "Country" }}<span class="sorting-icons"><i class="ti-arrows-vertical"></i></span></th>
                        <th class="contacts_col_last_active"><a class="js-ajax-ux-request js-list-sorting"
                                id="sort_last_seen" href="javascript:void(0)"
                                data-url="{{ urlResource('/address?action=sort&orderby=last_seen&sortorder=asc') }}">{{ "Postal Code" }}<span class="sorting-icons"><i class="ti-arrows-vertical"></i></span></th>
                ...
                <tbody id="addressb-td-container">
                    <!--ajax content here-->
                    @include('pages.addressb.components.table.ajax')
                    <!--ajax content here-->
                </tbody>
                <tfoot>
                    <tr>
