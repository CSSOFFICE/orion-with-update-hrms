...
<div class="col-md-12  col-lg-5 align-self-center text-right p-b-9  {{ $page['list_page_actions_size'] ?? '' }} {{ $page['list_page_container_class'] ?? '' }}"
    ...
    <div id="list-page-actions">
        ...
        <button type="button" data-toggle="tooltip" title="{{ cleanLang(__('lang.publish_estimate')) }}"
            class="list-actions-button d-none btn btn-page-actions waves-effect waves-dark confirm-action-info"
        ...
        @endif
        <a href="{{ url('projects/' . $bill->bill_projectid . '/variation')}}" class="btn btn-secondary list-actions-button btn btn-page-actions waves-effect waves-dark ">Back</a>
        @if($bill->is_project_creates=="No")
        <a class="list-actions-button btn btn-page-actions waves-effect waves-dark confirm-action-info" href="javascript:void(0)"
            data-confirm-title="Variation Order Approve"
            data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}"
            data-url="{{ url('project/' . $bill->bill_projectid . '/variation/'.$bill->vo_id)}}/approved">
            Approve</a>
        @endif
        <!--mark as revised-->
        ...
        @endif
        @if ($bill->bill_status == 'waiting_for_approval')
        <!--Approved from Management-->
        <button type="button" data-toggle="tooltip" title="Approved From Management"
            class="list-actions-button btn d-none  reset-target-modal-form btn-page-actions" id="status_change"
        ...
        <button type="button"
            class="list-actions-button d-none btn btn-page-actions waves-effect waves-dark actions-modal-button js-ajax-ux-request reset-target-modal-form"
            title="Change Status" data-toggle="modal" data-target="#actionsModal"
        ...
        <button type="button" data-toggle="tooltip" title="{{ cleanLang(__('lang.send_email')) }}"
            class="list-actions-button btn d-none btn-page-actions waves-effect waves-dark confirm-action-info"
        ...
        <span class="dropdown">
            ...
            <div class="dropdown-menu" aria-labelledby="listTableAction">
                ...
                <!--attach project-->
                <a class="dropdown-item confirm-action-danger d-none {{ runtimeVisibility('dettach-estimate', $bill->bill_projectid) }}"
                ...
                <!--deattach project-->
                <a class="dropdown-item actions-modal-button d-none js-ajax-ux-request reset-target-modal-form {{ runtimeVisibility('attach-estimate', $bill->bill_projectid) }}"
        ...
        <button type="button" data-toggle="tooltip" title="Delete Quotation"
            class="list-actions-button btn d-none btn-page-actions waves-effect waves-dark confirm-action-danger"
            data-confirm-title="Delete Variation Order" data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}"
