...
<tr id="project_{{ $project->project_id }}">
    ...
    <td class="projects_col_id"><a
            href={{ url('/projects/' . $project->project_id) }}>{{ $project->project_code??''}}</label></a>
    </td>
    <td class="projects_col_project">
        <a href={{ url('/projects/' . $project->project_id) }}>{{ str_limit($project->project_title ?? '---', 20) }}<a />
    </td>
    @if (config('visibility.projects_col_client'))
    <td class="projects_col_client">
        <a
            href={{ url('/clients/' . $project->project_clientid) }}>{{ str_limit($project->f_name ?? $project->client_company_name??'' , 12) }}</a>
    </td>
    @endif
    ...
    @endphp
    @if ($d->role_name == 'Super Admin' || $d->role_name == 'Manager')
    <td class="projects_col_status">
        <span
            class="label {{ runtimeProjectStatusColors($project->project_status, 'label') }}">{{ $project->project_sn }}</span>
    </td>
    @endif
    <td class="projects_col_action actions_column">
        ...
        <span class="list-table-action dropdown font-size-inherit">
            ...
            <button type="button" title="{{ cleanLang(__('lang.delete')) }}"
                ...
                data-url="{{ url('/') }}/projects/{{ $project->project_id }}" data-id="{{ $project->project_id }}">
            ...
            @endif
            <!--view-->
            <a href={{ url('/projects/' . $project->project_id) }} title="{{ cleanLang(__('lang.view')) }}"
