...
<tr id="estimate_{{ $estimate->bill_estimateid }}">
    ...
    <td class="estimates_col_id" id="estimates_col_id_{{ $estimate->bill_estimateid }}">
        <a href="{{ url('project/' . $estimate->bill_projectid . '/estimates/'.$estimate->vo_id)}}">{{$estimate->est_quotation_no}}</a>
    </td>
    ...
    <td class="estimates_col_company" id="estimates_col_company_{{ $estimate->bill_estimateid }}">
        <a href="/clients/{{ $estimate->bill_clientid }}">
            {{ str_limit($estimate->f_name ?? '---', 30) }}</a>
    </td>
    @endif -->
    ...
    <td class="estimates_col_created_by" id="estimates_col_created_by_{{ $estimate->bill_estimateid }}">
        ...
        {{ $estimate->f_name ?? runtimeUnkownUser() }}
    </td>
    @endif
    ...
    <td class="estimates_col_status" id="estimates_col_status_{{ $estimate->bill_estimateid }}">
        <span class="label {{ runtimeEstimateStatusColors($estimate->bill_status, 'label') }}">
            ...
            Management Approved
            @elseif($estimate->bill_status =="Approval_from_Management")
    ...
    <td class="estimates_col_action actions_column" id="estimates_col_action_{{ $estimate->bill_estimateid }}">
        ...
        <span class="list-table-action dropdown font-size-inherit">
            ...
            <button type="button" title="{{ cleanLang(__('lang.delete')) }}"
                ...
                data-url="{{ url('project/' . $estimate->bill_projectid . '/estimates/'.$estimate->vo_id)}}/delete">
                <i class="sl-icon-trash"></i>
            </button>
            @endif
            <!--edit-->
            @if(config('visibility.action_buttons_edit'))
            <a href="{{ url('project/' . $estimate->bill_projectid . '/estimates/'.$estimate->vo_id)}}/edit-estimate" title="{{ cleanLang(__('lang.edit')) }}"
                class="data-toggle-action-tooltip btn btn-outline-info btn-circle btn-sm">
                <i class="sl-icon-note"></i>
            </a>
            @endif
            <a href="{{ url('project/' . $estimate->bill_projectid . '/estimates/'.$estimate->vo_id)}}" title="{{ cleanLang(__('lang.view')) }}"
                class="data-toggle-action-tooltip btn btn-outline-info btn-circle btn-sm">
        ...
        <!--action button-->
        @if($estimate->is_project_creates=="No")
        <span class="list-table-action dropdown  font-size-inherit">
            ...
            <div class="dropdown-menu" aria-labelledby="listTableAction">
                <a class="dropdown-item confirm-action-info" href="javascript:void(0)"
                    data-confirm-title="Variation Order Approved"
                    data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}"
                    data-url="{{ url('project/' . $estimate->bill_projectid . '/variation/'.$estimate->vo_id)}}/approved">
                    Approve</a>
            </div>
        </span>
        @endif
        <!--more button (team)-->
        @if(config('visibility.action_buttons_edit') == 'showy')
        <span class="list-table-action dropdown d-none font-size-inherit">
