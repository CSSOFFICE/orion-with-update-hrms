<div class="board">
    <div class="board-body {{ runtimeKanbanBoardColors($board['name'], 'tasks') }}">
        <div class="board-heading clearfix">
            ...
            <div class="pull-right x-action-icons">
                ...
                <span class="edit-add-modal-button js-ajax-ux-request reset-target-modal-form cursor-pointer"
                    ...
                    data-url="{{ urlResource('/quo/create?status='.$board['name']) }}"
                    data-loading-target="commonModalBody" data-modal-title="{{ "Quotation" }}"
                    data-action-url="{{ urlResource('/quo?type=kanban') }}" data-action-method="POST"
        ...
        <!--cards-->
        <div class="content kanban-content" id="kanban-board-wrapper-{{ $board['name'] }}" data-board-name="{{ $board['name'] }}">

            <!--dynamic content-->

            @if(@count($board['tasks']) > 0)
            @include('pages.quos.components.kanban.card')
            @endif
