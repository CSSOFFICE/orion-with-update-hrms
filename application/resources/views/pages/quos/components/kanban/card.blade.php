@foreach ($board['tasks'] as $task)
    <a class="" href="{{ url('/estimates/' . $task->view_id) }}" data-loading-target="main-top-nav-bar"><span
            class="x-strike-through" id="table_task_title_{{ $task->task_id }}">
           </span>
    <!--each card-->
    <div class="kanban-card "

        data-task-id="{{ $task->task_id }}" data-loading-target="main-top-nav-bar" id="card_task_{{ $task->task_id }}">
        <div class="x-title wordwrap" id="kanban_task_title_{{ $task->task_id }}">{{ $task->quo_number }}
        ...
        <div class="x-meta">
            ...
            @if (config('system.settings_tasks_kanban_priority') == 'show')
                <label
                    class="label {{ runtimeTaskPriorityColors($task->task_priority, 'label') }} p-t-3 p-b-3 p-l-8 p-r-8"
                    data-toggle="tooltip"
                    title="{{ cleanLang(__('lang.priority')) }}">{{ runtimeLang($task->task_title) }}</label>
            @endif
            <!--date created-->
