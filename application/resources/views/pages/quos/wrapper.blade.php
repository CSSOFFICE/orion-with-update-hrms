...
<div class="container-fluid">
    ...
    <div class="row kanban-wrapper">
        <div class="col-12" id="tasks-layout-wrapper">
            ...
            @include('pages.quos.components.kanban.wrapper')
            @else
            <!--tasks table-->
            @include('pages.quos.components.table.wrapper')
            <!--tasks table-->
            @endif
            <!--filter-->
            @if(auth()->user()->is_team)
...
<a href="javascript:void(0)" id="dynamic-task-content"
    ...
    data-target="#cardModal" data-url="{{ url('/quo/'.request()->route('quo').'?ref=list') }}"
