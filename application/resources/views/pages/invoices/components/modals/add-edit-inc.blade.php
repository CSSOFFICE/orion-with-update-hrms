...
<div class="row" id="js-trigger-invoices-modal-add-edit" data-payload="{{ $page['section'] ?? '' }}">
    <div class="col-lg-12">
        ...
        <div class="form-group row">
            ...
            <select class="form-control" name="Billing_Address" id="Billing_Address">
                ...
                <?php foreach ($billing as $k => $billing) { ?>
                    <option value="<?php echo $billing->id; ?>">{{$billing->street}}</option>
                <?php } ?>
            </select>
        </div>
        <div class="form-group row">
            ...
            <select class="form-control" name="Shipping_Address" id="Shipping_Address">
                ...
                <?php foreach ($Shipping as $k => $Shipping) { ?>
                    <option value="<?php echo $Shipping->id; ?>">{{$Shipping->street}}</option>
                <?php } ?>
            </select>
        </div>
        <div class="form-group row">
        ...
        <input type="hidden" name="client_id" value="{{$client[0]->project_clientid}}">
        <input type="hidden" name="project_id" value="{{$client[0]->project_id}}">
        <input type="hidden" name="total_invoice_amount" value="{{$total_invoice_amount}}">
        {{-- <input type="hidden" name="quotation_amount" value="{{$quotation[0]->bill_final_amount}}"> --}}
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-12 col-lg-9">
                <select name="payment_terms" class="form-control" placeholder="Terms" id="terms">
                    ...
                    <option value="{{ $terms->payment_term_id}}">{{$terms->payment_term}}</option>
                    @endforeach
                </select>
            </div>
        </div>
    ...
    <script>
        ...
        $("#commonModalSubmitButton").unbind().click(function(e) {
            ...
            let url = "{{ urlResource('/invoices/create') }}";
