...
<div class="row" id="js-trigger-invoices-modal-add-edit" data-payload="{{ $page['section'] ?? '' }}">
    <div class="col-lg-12">
        ...
        <div class="row">
            <div class="col-6">
                <div class="form-group row">
                    <label>Project Name : {{$project->project_title??''}}</label>

                </div>
            </div>
            <div class="col-6">
                <div class="form-group row">
                    <label>Client Name : {{$project->f_name??$project->client_company_name}}</label>
        ...
        <div class="form-group row">
            ...
            <select class="form-control" name="Billing_Address" id="Billing_Address">
                ...
                <?php foreach ($billing as $k => $billing) { ?>
                    <option value="<?php echo $billing->id; ?>" {{($invoice->Billing_Address==$billing->id)?'selected':''}}>{{$billing->street}}</option>
                <?php } ?>
            </select>
        </div>
        <div class="form-group row">
            ...
            <select class="form-control" name="Shipping_Address" id="Shipping_Address">
                ...
                <?php foreach ($Shipping as $k => $Shipping) { ?>
                    <option value="<?php echo $Shipping->id; ?>" {{($invoice->Shipping_Address==$Shipping->id)?'selected':''}}>{{$Shipping->street}}</option>
                <?php } ?>
            </select>
        </div>
        <div class="form-group row">
            ...
            <select class="form-control" name="category_id_prili" id="category_id_prili">
                ...
                <?php foreach ($templete_category as $k => $purchase) { ?>
                    <option value="<?php echo $purchase->milestonecategory_id; ?>" {{($invoice->mile_id==$purchase->milestonecategory_id)?'selected':''}}><?php echo $purchase->milestonecategory_title; ?></option>
                <?php } ?>
            </select>
        </div>
        <div class="form-group row">
            ...
            <select class="form-control" name="pp" id="Task_id_quotation">
                @foreach($grn_data as $tas)
                @if($tas->task_cat_id==$invoice->mile_id)
                <option value="<?php echo $tas->task_id; ?>" {{($tas->task_id==$invoice->task)?'selected':''}}><?php echo $tas->task_title; ?></option>
                @endif
                @endforeach
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-12 col-lg-9">
                <input type="text" class="form-control  form-control-sm pickadate" id="invoice_date" name="bill_date_add_edit" autocomplete="off"
                    value="{{ runtimeDatepickerDate($invoice->invoice_date ?? '') }}">
        ...
        <input type="hidden" name="client_id" value="{{$project->project_clientid}}">
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-12 col-lg-9">
                <select name="payment_terms" class="form-control" placeholder="Terms" id="terms">
                    ...
                    <option value="{{ $terms->payment_term_id}}" {{($invoice->terms==$terms->payment_term_id)?'selected':''}}>{{$terms->payment_term}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-12 col-lg-9">
                <input type="text" id="due_date" class="form-control form-control-sm pickadate" name="bill_due_date_add_edit"
                    autocomplete="off" value="{{ runtimeDatepickerDate($invoice->invoice_due_date ?? '') }}">
        ...
        <div class="col-12">
            ...
            <div class="table-responsive my-3 purchaseTable">
                ...
                <table class="table">
                    ...
                    <tbody class="AddItem" id="vendor_items_table1">
                        ...
                        @foreach($invoice_item as $i=>$invoice)
                        @if($invoice->item_type=="product")
                        <tr>
                            <td>{{$i+1}}</td>
                            <td style="min-width:200px">
                                <select name="u_item[{{$i}}]" id="product" class="form-control">
                                    @foreach ($product as $productT)

                                    <option value="{{ $productT->product_id }}" {{($invoice->item==$productT->product_id??'')?'selected':''}}>{{ $productT->product_name }}</option>
                                    @endforeach
                                </select>

                                <input type='hidden' class='form-control' name='type[{{$i}}]' id='type_{{$i}}' value='product'>
                            </td>
                            <td style="min-width:100px"><input type='text' class='form-control' value="{{$invoice->item_qtn}}" name='quantity[{{$i}}]' id='quantity_{{$i}}' oninput='updateRowTotal(this)'></td>
                            <td style="min-width:100px"><input type='text' class='form-control' value="{{$invoice->unit}}" name='unit[{{$i}}]' id='unit_{{$i}}'></td>
                            <td style="min-width:100px"><input type='text' class='form-control' value="{{$invoice->rate}}" name='rate[{{$i}}]' id='rate_{{$i}}' oninput='updateRowTotal(this)'></td>
                            <td style="min-width:100px"><input type='text' class='form-control' value="{{$invoice->total}}" name='total[]' id='total_{{$i}}' readonly></td>
                            <td>
                        ...
                        <tr>
                            <td>{{$i+1}}</td>
                            <td style="min-width:200px">
                                <input type='text' class='form-control' id="item_description_{{$i}}" name='item_description[{{$i}}]' value="{{$invoice->job_description}}">

                                <input type='hidden' class='form-control' name='type[{{$i}}]' id='type_{{$i}}' value='plain'>
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    ...
                    <input type="checkbox" id="inclusive_gst" name="inclusive_gst" {{($tags=='on')?'checked':''}}>
                    ...
                    <div id="gst_box">
                        ...
                        <select class="form-control" id="order_gst2" name="order_gst2">
                            ...
                            foreach ($all_gst as $gst) { ?>
                                <option value="<?php echo $gst->gst ?>" {{($gst12 == $gst->gst)?'selected':''}}><?php echo $gst->gst ?></option>
                            <?php } ?>
                        </select>
    ...
    <script>
        ...
        $("#commonModalSubmitButton").unbind().click(function(e) {
            ...
            let url = "{{ urlResource('/invoices/'.$invoice->invoice_id.'/update') }}";
