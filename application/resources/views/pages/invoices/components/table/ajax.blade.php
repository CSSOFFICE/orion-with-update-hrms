...
<!--each row-->
<tr id="invoice_{{ $invoice->invoice_id  }}">
    @if(config('visibility.invoices_col_checkboxes'))
    <td class="invoices_col_checkbox checkitem" id="invoices_col_checkbox_{{ $invoice->invoice_id }}">
        <!--list checkbox-->
        <span class="list-checkboxes display-inline-block w-px-20">
            <input type="checkbox" id="listcheckbox-invoices-{{ $invoice->invoice_id }}"
                name="ids[{{ $invoice->invoice_id }}]"
            ...
            <label for="listcheckbox-invoices-{{ $invoice->invoice_id }}"></label>
        </span>
    </td>
    @endif
    ...
    <td class="invoices_col_date" id="invoices_col_date_{{ $invoice->invoice_id }}">
        {{ runtimeDate($invoice->invoice_date) }}
    </td>
    ...
    <td class="invoices_col_amount" id="invoices_col_amount_{{ $invoice->invoice_id }}">
        {{ runtimeMoneyFormat($invoice->total) }}
    </td>
    @if(config('visibility.invoices_col_payments'))
    <td class="invoices_col_payments" id="invoices_col_payments_{{ $invoice->invoice_id }}">
        <a
            href="{{ url('payments?filter_payment_invoiceid='.$invoice->bill_invoiceid) }}">{{ runtimeMoneyFormat($invoice->total) }}</a>
    </td>
    @endif
    ...
    <td class="invoices_col_action actions_column" id="invoices_col_action_{{ $invoice->invoice_id }}">
        <!--action button-->
        <span class="list-table-action dropdown font-size-inherit">
            ...
            <a title="{{ cleanLang(__('lang.download')) }}" title="{{ cleanLang(__('lang.download')) }}"
                ...
                href="{{ url('/invoices/'.$invoice->invoice_id.'/pdf') }}" download>
            ...
            <button type="button" title="{{ cleanLang(__('lang.delete')) }}"
                ...
                data-url="{{ url('/') }}/invoices/{{ $invoice->invoice_id }}/delete">
                <i class="sl-icon-trash"></i>
            </button>
            @endif
            <!--edit-->
            @if(config('visibility.action_buttons_edit'))
            <!-- <a href="{{ url('/invoices/'. $invoice->invoice_id.'/edit-invoice') }}" title="{{ cleanLang(__('lang.edit')) }}"
            ...
            <button type="button" title="{{ cleanLang(__('lang.edit')) }}"
                ...
                data-url="{{ urlResource('/invoices/'.$invoice->invoice_id.'/edit') }}"
                data-loading-target="commonModalBody" data-modal-title="{{ cleanLang(__('lang.edit_invoice')) }}"
                data-action-url="{{ urlResource('/invoices/'.$invoice->invoice_id.'?ref=list') }}"
                data-action-method="PUT" data-action-ajax-class=""
                data-action-ajax-loading-target="commonModalBody">
                <i class="sl-icon-note"></i>
            </button>
            @endif
            <!-- <a href="{{ url('/invoices/'. $invoice->invoice_id) }}" title="{{ cleanLang(__('lang.view')) }}"
                class="data-toggle-action-tooltip btn btn-outline-info btn-circle btn-sm">
