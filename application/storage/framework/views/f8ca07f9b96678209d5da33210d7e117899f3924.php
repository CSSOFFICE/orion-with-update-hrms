    ...
    <table class="table table-bordered border-secondary nowrap w-100">
        ...
        <tbody id="insurance-table-body">
            <tr style="background-color:#FEF2CB">
                ...
                <th class="text-start" style="text-align:left!important;"><b>BILL NO. 2 - INSURANCES FOR THE WORKS</b>
                </th>
                <th class="text-start" colspan="12"></th>
                <th class="text-start" style="text-align:left!important;">$<span
                        id="grand-total"class="insurance-gt"></span>
                </th>
                </th>
            ...
            <?php if(config('visibility.bill_mode') == 'viewing'): ?>
                <?php
                $insuranceTotal = DB::table('quotation_templates')->where('template_id', 2)->where('quotation_no', $bill->est_quotation_no)->sum('total');
                
                $insuranceAmount = DB::table('quotation_templates')->where('template_id', 2)->where('quotation_no', $bill->est_quotation_no)->sum('amount');
                ...
                <?php $__currentLoopData = $quotation_templates; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $data): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php if($data->template_id == 2 && $data->quotation_no == $page['crumbs'][2]): ?>
                    ...
                    <?php if($data->template_id == 2 && $data->quotation_no == $page['crumbs'][2]): ?>
                        <?php if($data->type == 'head'): ?>
                            <tr class="fw-bold" style="background-color:#E2EFD9">
                                ...
                                <td>
                                    <select class="form-control unit-input" name="unit[]" style="width:fit-content;">
        ...
        <tfoot>
            <tr id="subtotal-row" style="border-bottom: 1px solid black;border-top: 2px solid black;">
                ...
                <td id="amount-subtotal">$<?php echo e(number_format($insurance['total'], 2)); ?></td>
                <script>
                    $(document).ready(function() {
                        $('.insurance-gt').text('<?php echo number_format($insurance['total'], 2); ?>');
                    });
                </script>
            </tr>
            <tr style="border-bottom: 2px solid black;border-top: 1px solid black;">
                ...
                <td>$<?php echo e(number_format($insurance['total'], 2)); ?></td>
            </tr>
        </tfoot>
    </table>
</div>
<?php if(config('visibility.bill_mode') == 'editing'): ?>
    ...
    <button type="button" id="insurance_head_line" class="btn btn-secondary btn-rounded btn-sm btn-rounded-icon">
        <i class="mdi mdi-plus-circle-outline text-themecontrast"></i>
        <span>Heading</span>
    </button>
<?php endif; ?>
<script>
    ...
    $(document).ready(function() {
        ...
        $('#insurance_new_blank_line').on('click', function() {
            $('#insurance-table-body').append(getRowTemplate('row', uomOptions));
        });

        // Add new head line (appends to end)
        $('#insurance_head_line').on('click', function() {
            $('#insurance-table-body').append(getRowTemplate('head', uomOptions));
        });

        // Add row after specific row/head
        $('#insurance-table-body').on('click', '.add-row-after', function() {
            $(this).closest('tr').after(getRowTemplate('row', uomOptions));
        });

        // Add head after specific row/head
        $('#insurance-table-body').on('click', '.add-head-after', function() {
...
<?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/orion/application/resources/views/pages/bill/components/elements/templates/insurance.blade.php ENDPATH**/ ?>