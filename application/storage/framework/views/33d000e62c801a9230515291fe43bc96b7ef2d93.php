    ...
    <table class="table table-bordered border-secondary nowrap w-100">
        ...
        <tbody id="preliminaries-table-body">
            ...
            <?php if(config('visibility.bill_mode') == 'viewing'): ?>
                <?php
                $preliTotal = DB::table('quotation_templates')->where('template_id', 1)->where('quotation_no', $bill->est_quotation_no)->sum('total');
                
                $preliAmount = DB::table('quotation_templates')->where('template_id', 1)->where('quotation_no', $bill->est_quotation_no)->sum('amount');
        ...
        <tfoot>
            <tr id="subtotal-row" style="border-bottom: 1px solid black;border-top: 2px solid black;">
                ...
                <td id="amount-subtotal">$<?php echo e(number_format($preli['total'], 2)); ?></td>
                <script>
                    $(document).ready(function() {
                        $('.preli-gt').text('<?php echo number_format($preli['total'], 2); ?>');
                    });
                </script>
            </tr>
            <tr style="border-bottom: 2px solid black;border-top: 1px solid black;">
                ...
                <td>$<?php echo e(number_format($preli['total'], 2)); ?></td>
            </tr>
        </tfoot>
...
<?php if(config('visibility.bill_mode') == 'editing'): ?>
    ...
    <button type="button" id="preliminaries_new_head_line"
        class="btn btn-secondary btn-rounded btn-sm btn-rounded-icon">
        <i class="mdi mdi-plus-circle-outline text-themecontrast"></i>
        <span>Heading</span>
    </button>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#preliminaries-billing-time-actions-blank').on('click', function() {
            $('#preliminaries-table-body').append(getRowTemplate('row', uomOptions));
        });

        // Add new head line (appends to end)
        $('#preliminaries_new_head_line').on('click', function() {
            $('#preliminaries-table-body').append(getRowTemplate('head', uomOptions));
        });

        // Add row after specific row/head
        $('#preliminaries-table-body').on('click', '.add-row-after', function() {
            $(this).closest('tr').after(getRowTemplate('row', uomOptions));
        });

        // Add head after specific row/head
        $('#preliminaries-table-body').on('click', '.add-head-after', function() {
...
<?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/orion/application/resources/views/pages/bill/components/elements/templates/preliminaries.blade.php ENDPATH**/ ?>