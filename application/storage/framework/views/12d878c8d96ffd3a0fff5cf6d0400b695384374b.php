    ...
    <table class="table table-bordered border-secondary nowrap w-100">
        ...
        <tbody id="plumbing-table-body">
            <tr style="background-color:#FEF2CB">
                ...
                <th class="text-start" style="text-align:left!important;"><b>BILL NO. 4 - PROPOSED PLUMBING & SANITARY
                        WORKS</b></th>
                <th class="text-start" colspan="12"></th>
                <th class="text-start" style="text-align:left!important;">$<span id="grand-total"
                        class="plumbing-gt"></span>
                </th>
            </tr>
            <?php if(config('visibility.bill_mode') == 'viewing'): ?>
                <?php
                
                $plumbingTotal = DB::table('quotation_templates')->where('template_id', 4)->where('quotation_no', $bill->est_quotation_no)->sum('total');
                
                $plumbingAmount = DB::table('quotation_templates')->where('template_id', 4)->where('quotation_no', $bill->est_quotation_no)->sum('amount');
                ...
                <?php $__currentLoopData = $quotation_templates; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $data): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php if($data->template_id == 4 && $data->quotation_no == $page['crumbs'][2]): ?>
        ...
        <tfoot>
            <tr id="subtotal-row" style="border-bottom: 1px solid black;border-top: 2px solid black;">
                ...
                <td id="amount-subtotal">$<?php echo e(number_format($plumbing['total'], 2)); ?></td>
                <script>
                    $(document).ready(function() {
                        $('.plumbing-gt').text('<?php echo number_format($plumbing['total'], 2); ?>');
                    });
                </script>
            </tr>
            <tr style="border-bottom: 2px solid black;border-top: 1px solid black;">
                ...
                <td>$<?php echo e(number_format($plumbing['total'], 2)); ?></td>
            </tr>
        </tfoot>
    </table>
</div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#plumbing_new_blank_line').on('click', function() {
            $('#plumbing-table-body').append(getRowTemplate('row', uomOptions));
        });

        // Add new head line (appends to end)
        $('#plumbing_new_head_line').on('click', function() {
            $('#plumbing-table-body').append(getRowTemplate('head', uomOptions));
        });

        // Add row after specific row/head
        $('#plumbing-table-body').on('click', '.add-row-after', function() {
            $(this).closest('tr').after(getRowTemplate('row', uomOptions));
        });

        // Add head after specific row/head
        $('#plumbing-table-body').on('click', '.add-head-after', function() {
...
<?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/orion/application/resources/views/pages/bill/components/elements/templates/plumbing_sanity.blade.php ENDPATH**/ ?>