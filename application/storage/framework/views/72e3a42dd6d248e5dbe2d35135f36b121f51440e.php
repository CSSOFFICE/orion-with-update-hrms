...
<button type="button" id="plumbing_new_blank_line" class="btn btn-secondary btn-rounded btn-sm btn-rounded-icon">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
       document.getElementById('plumbing_new_blank_line').addEventListener('click', function() {
        const tableBody = document.getElementById('plumbing-table-body');

        const plumbingnewRow = document.createElement('tr');
        plumbingnewRow.innerHTML = `
            <td>
        ...
        tableBody.appendChild(plumbingnewRow);

        // Add event listeners to the new row's inputs
        plumbingnewRow.querySelectorAll('.rate-input, .qty-input').forEach(input => {
            input.addEventListener('input', updateTotals);
        });

        plumbingnewRow.querySelector('.delete-row-btn').addEventListener('click', function() {
            plumbingnewRow.remove();
        });
    });
});
...
<?php /**PATH /www/wwwroot/orion.braincave.work/application/resources/views/pages/bill/components/elements/templates/plumbing_sanity.blade.php ENDPATH**/ ?>