    ...
    <table class="table table-bordered border-secondary">
        <thead>
            ...
            <tr>
                ...
                <th class="text-start">BILL NO. 3 - PROPOSED BUILDING WORKS</th>
...
<button type="button" id="schedule_new_blank_line" class="btn btn-secondary btn-rounded btn-sm btn-rounded-icon">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
       document.getElementById('schedule_new_blank_line').addEventListener('click', function() {
        const tableBody = document.getElementById('schedule-table-body');

        const schedulenewRow = document.createElement('tr');
        schedulenewRow.innerHTML = `
            <td>
        ...
        tableBody.appendChild(schedulenewRow);

        // Add event listeners to the new row's inputs
        schedulenewRow.querySelectorAll('.rate-input, .qty-input').forEach(input => {
            input.addEventListener('input', updateTotals);
        });

        schedulenewRow.querySelector('.delete-row-btn').addEventListener('click', function() {
            schedulenewRow.remove();
        });
    });
});
...
<?php /**PATH /www/wwwroot/orion.braincave.work/application/resources/views/pages/bill/components/elements/templates/schedule_of_works.blade.php ENDPATH**/ ?>