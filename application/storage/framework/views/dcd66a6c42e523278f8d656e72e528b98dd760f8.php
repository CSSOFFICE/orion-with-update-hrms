...
<table class="table table-bordered border-secondary nowrap w-100">
    ...
    <tbody id="pcps-table-body">
        <tr style="background-color:#FEF2CB">
            ...
            <th class="text-start" style="text-align:left!important; word-wrap:break-word; white-space:normal;"><b>BILL NO. 7 - Provide the following Prime Cost
                    &amp; Provisional Sums for works to be executed by Nominated Suppliers &amp; Sub-Contractors or
                    expended by Architect as directed. The sums shall be adjusted in due course.</b></th>
            <th class="text-start" colspan="12"></th>
            <th class="text-start" style="text-align:left!important;">$<span id="grand-total" class="pcps-gt"></span>
            </th>
            </th>
        ...
        <?php
        $pcpsTotal = DB::table('quotation_templates')->where('template_id', 7)->where('quotation_no', $bill->est_quotation_no)->sum('total');

        $pcpsAmount = DB::table('quotation_templates')->where('template_id', 7)->where('quotation_no', $bill->est_quotation_no)->sum('amount');
        ...
        <?php if($data->template_id == 7 && $data->quotation_no == $page['crumbs'][2]): ?>
    ...
    <tfoot>
        <tr id="subtotal-row" style="border-bottom: 1px solid black;border-top: 2px solid black;">
            ...
            <td id="amount-subtotal">$<?php echo e(number_format($pcps['total'], 2)); ?></td>
            <script>
                $(document).ready(function() {
                    $('.pcps-gt').text('<?php echo number_format($pcps['total'], 2); ?>');
                });
            </script>
        </tr>
        <tr style="border-bottom: 2px solid black;border-top: 1px solid black;">
            ...
            <td>$<?php echo e(number_format($pcps['total'], 2)); ?></td>
        </tr>
    </tfoot>
</table>
</div>
...
<?php if(config('visibility.bill_mode') == 'editing'): ?>
<button type="button" id="pcps_new_blank_line" class="btn btn-secondary btn-rounded btn-sm btn-rounded-icon">
    <i class="mdi mdi-plus-circle-outline text-themecontrast"></i>
    <span><?php echo e(cleanLang(__('lang.new_blank_line'))); ?></span>
</button>
<button type="button" id="pcps_new_head_line" class="btn btn-secondary btn-rounded btn-sm btn-rounded-icon">
    <i class="mdi mdi-plus-circle-outline text-themecontrast"></i>
    <span>Heading</span>
</button>
<?php endif; ?>
<script>
    ...
    $(document).ready(function() {
        ...
        $('#pcps_new_blank_line').on('click', function() {
            $('#pcps-table-body').append(getRowTemplate('row', uomOptions));
        });

        // Add new head line (appends to end)
        $('#pcps_new_head_line').on('click', function() {
            $('#pcps-table-body').append(getRowTemplate('head', uomOptions));
        });

        // Add row after specific row/head
        $('#pcps-table-body').on('click', '.add-row-after', function() {
            $(this).closest('tr').after(getRowTemplate('row', uomOptions));
        });

        // Add head after specific row/head
        $('#pcps-table-body').on('click', '.add-head-after', function() {
...
<?php /**PATH C:\xampp\htdocs\orion\application\resources\views/pages/bill/components/elements/templates/pc_ps_sums.blade.php ENDPATH**/ ?>