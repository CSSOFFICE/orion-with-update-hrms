...
<button type="button" id="insurance_new_blank_line" class="btn btn-secondary btn-rounded btn-sm btn-rounded-icon">
...
<script>
    ...
    document.getElementById('insurance_new_blank_line').addEventListener('click', function() {
        const tableBody = document.getElementById('insurance-table-body');

        const insurancenewRow = document.createElement('tr');
        insurancenewRow.innerHTML = `
            <td>
        ...
        tableBody.appendChild(insurancenewRow);

        // Add event listeners to the new row's inputs
        newRow.querySelectorAll('.rate-input, .qty-input').forEach(input => {
            input.addEventListener('input', updateTotals);
        });
    ...
    // Add event listener for the delete button in existing rows
        ...
        document.querySelectorAll('.delete-row-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                if (confirm('Are you sure you want to delete this row?')) {
                    // Make an AJAX request to delete the data from the server
                    fetch(url, {
                        method: 'DELETE',
                        headers: {
...
<?php /**PATH /www/wwwroot/orion.braincave.work/application/resources/views/pages/bill/components/elements/templates/insurance.blade.php ENDPATH**/ ?>