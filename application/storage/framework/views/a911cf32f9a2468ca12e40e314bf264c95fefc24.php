...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
       document.getElementById('pcps_new_blank_line').addEventListener('click', function() {
        const tableBody = document.getElementById('pcps-table-body');

        const pcpsnewRow = document.createElement('tr');
        pcpsnewRow.innerHTML = `
        <td>
        ...
        tableBody.appendChild(pcpsnewRow);

        // Add event listeners to the new row's inputs
        pcpsnewRow.querySelectorAll('.rate-input, .qty-input').forEach(input => {
            input.addEventListener('input', updateTotals);
        });

        pcpsnewRow.querySelector('.delete-row-btn').addEventListener('click', function() {
            pcpsnewRow.remove();
        });
    });
});
...
<?php /**PATH /www/wwwroot/orion.braincave.work/application/resources/views/pages/bill/components/elements/templates/pc_ps_sums.blade.php ENDPATH**/ ?>